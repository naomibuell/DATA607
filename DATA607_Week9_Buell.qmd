...
First, I sign up for an API key on the New York Times website. Here, I use R Studio API `askForPassword()` function to keep my API key private. The base HTTP request URL is defined below as well. I will be using the Article Search API to get New York Times movie reviews, so I define the filter query accordingly.
...
```{r}
#| label: load data to df

pages <- c(1:10) # set num pages to iterate through
movies_df <- data_frame() # initialize data frame

for (page in pages) {
  movies_df <-
    get_movies(filter, page) |> # call function
    rbind(movies_df, .data) # append to df
...
Next I subset the data frame to several variables of interest and do some variable cleaning before I finalize this data set.
