...
There is a very weak negative correlation between the binary variable indicating whether a state had Medicaid expansion and the prevalence of opioid use disorder (r = `r round(r, 2)`)--as the presence of Medicaid expansion increases (coded as 1), the prevalence of opioid misuse tends to decrease, and vice versa. Only `r round(r_sqrd*100, 0)`% of variance in opioid misuse prevalence can be explained by Medicaid expansion; the remaining `r round((1-r_sqrd)*100, 0)`% of the variance may be influenced by other factors not considered in this analysis. Though there may be a slight tendency for states with Medicaid expansion to have lower opioid misuse prevalence, the relationship is not strong enough to make definitive conclusions.
...
```{r}
#| label: world population density x population correlation
...
There is a very weak correlation between population density (at the single point in time when data was pulled) and population over time (r = `r round(r, 2)`). Only `r round(r_sqrd*100, 0)`% of variance in population can be explained by density. Since this is very close to zero, there is little evidence of a relationship between population and density in this data.
